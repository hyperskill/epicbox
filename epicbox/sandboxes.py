...
from epicbox import config, exceptions, utils
...
class Sandbox:
    ...
    def __repr__(self) -> str:
        return f"<Sandbox: {self.id_} container={self.container.short_id}>"
...
def create(
    ...
    command_list: list[str] = ["/bin/sh", "-c", cmd]
...
def start(sandbox: Sandbox, stdin: bytes | str | None = None) -> dict[str, Any]:
    ...
    try:
        stdout, stderr = utils.docker_communicate(
            sandbox.container,
            stdin=stdin,
            timeout=sandbox.realtime_limit,
        )
    except TimeoutError:
...
def destroy(sandbox: Sandbox) -> None:
    ...
    try:
        ...
        logger.exception("Failed to destroy the sandbox container", sandbox=sandbox)
    else:
...
def run(
    ...
    Destroy the sandbox when it has finished running.
...
@contextmanager
def working_directory() -> Iterator[WorkingDirectory]:
    docker_client = utils.get_docker_client()
...
def _write_files(container: Container, files: list[dict[str, Any]]) -> None:
